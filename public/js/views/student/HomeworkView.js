...
var HomeworkView = {
  ...
  template: `
    <div class="max-w-6xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">平时作业</h1>
          <p class="text-gray-500 mt-1">每章课后作业，巩固理论知识</p>
        </div>
      </div>
      ...
      <div v-if="!loading && activeTab === 'pending'" class="space-y-6">
        <div v-for="group in groupedByChapter" :key="group.chapter" class="space-y-3">
          <div class="flex items-center gap-2">
            <h2 class="text-base font-semibold text-gray-700">{{ group.chapter }}</h2>
            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">{{ group.items.length }}</span>
          </div>
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-2">
                  <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">课后作业</span>
        ...
        <div v-if="pendingAssignments.length === 0" class="text-center py-16">
          ...
          <p class="text-gray-400">太棒了！所有平时作业都已提交</p>
        </div>
      </div>
  ...
  computed: {
    homeworkAssignments() {
      return this.assignments.filter(function(a) { return a.type === 'homework'; });
    },
    pendingAssignments() {
      var submittedIds = new Set(this.submissions.map(function(s) { return s.assignment_id; }));
      return this.homeworkAssignments.filter(function(a) { return !submittedIds.has(a.id); });
    },
    groupedByChapter() {
      var map = {};
      var order = [];
      this.pendingAssignments.forEach(function(a) {
        var ch = a.chapter_title || ('第' + (a.chapter_id || '?') + '章');
        if (!map[ch]) {
          map[ch] = [];
          order.push(ch);
        }
      ...
      return order.map(function(ch) {
        return { chapter: ch, items: map[ch] };
      });
    },
    homeworkSubmissions() {
      var homeworkIds = new Set(this.homeworkAssignments.map(function(a) { return a.id; }));
      return this.submissions.filter(function(s) { return homeworkIds.has(s.assignment_id); });
    },
    submittedSubmissions() {
      return this.homeworkSubmissions.filter(function(s) { return s.status === 'pending' || s.status === 'reviewed'; });
    },
    gradedSubmissions() {
      return this.homeworkSubmissions.filter(function(s) { return s.status === 'graded' || s.status === 'returned'; });
    },
    tabs() {
      return [
