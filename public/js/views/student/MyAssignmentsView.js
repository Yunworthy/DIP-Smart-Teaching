/* ============================================================
 *  MyAssignmentsView.js — 我的作业（学生端）
 * ============================================================ */
var MyAssignmentsView = {
  name: 'MyAssignmentsView',
  template: `
    <div class="max-w-6xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">我的作业</h1>
          <p class="text-gray-500 mt-1">查看、提交和追踪你的课程作业</p>
        </div>
      </div>
      ...
      <!-- Pending Tab -->
      <div v-if="!loading && activeTab === 'pending'" class="space-y-6">
        <div v-for="group in groupedPending" :key="group.type" class="space-y-3">
          <div class="flex items-center gap-2">
            <h2 class="text-base font-semibold text-gray-700">{{ group.label }}</h2>
            <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="group.type === 'homework' ? 'bg-blue-50 text-blue-700' : 'bg-green-50 text-green-700'">{{ group.items.length }}</span>
          </div>
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-2">
                  <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="typeBadge(a.type)">{{ typeName(a.type) }}</span>
        ...
        <div v-if="pendingAssignments.length === 0" class="text-center py-16">
          ...
          <p class="text-gray-400">太棒了！所有作业都已提交</p>
        </div>
      </div>
      ...
      <div v-if="!loading && activeTab === 'submitted'" class="space-y-4">
        <div v-for="s in submittedSubmissions" :key="s.id"
          ...
          <div class="flex items-start justify-between">
            <div>
              <h3 class="text-lg font-semibold text-gray-800">{{ s.assignment_title || '作业' }}</h3>
      ...
      <div v-if="showSubmitModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showSubmitModal = false">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
          <h2 class="text-xl font-bold text-gray-800 mb-4">提交作业</h2>
          <p class="text-sm text-gray-500 mb-4">{{ submittingAssignment?.title }}</p>
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">内容摘要</label>
              <textarea v-model="submitForm.content" rows="5" required
                        ...
                        placeholder="记录处理流程、参数、现象和结论..."></textarea>
            </div>
            <div>
  ...
  computed: {
    pendingAssignments() {
      const submittedIds = new Set(this.submissions.map(s => s.assignment_id));
      return this.assignments.filter(a => !submittedIds.has(a.id));
    },
    groupedPending() {
      var groups = [];
      var typeOrder = ['homework', 'lab_report'];
      var typeLabels = { homework: '平时作业', lab_report: '实验报告' };
      var map = {};
      this.pendingAssignments.forEach(function(a) {
      ...
      typeOrder.forEach(function(t) {
        if (map[t] && map[t].length > 0) {
          groups.push({ type: t, label: typeLabels[t] || t, items: map[t] });
        }
      });
      // catch any types not in typeOrder
      Object.keys(map).forEach(function(t) {
        if (typeOrder.indexOf(t) === -1 && map[t].length > 0) {
          groups.push({ type: t, label: typeLabels[t] || t, items: map[t] });
        }
      });
      return groups;
    ...
    submittedSubmissions() {
      return this.submissions.filter(s => s.status === 'pending' || s.status === 'reviewed');
    },
    gradedSubmissions() {
      return this.submissions.filter(s => s.status === 'graded' || s.status === 'returned');
    },
    tabs() {
      return [
  ...
  async mounted() {
    try {
      ...
      this.assignments = assignments.status === 'fulfilled' ? (Array.isArray(assignments.value) ? assignments.value : (assignments.value.assignments || [])) : [];
      this.submissions = submissions.status === 'fulfilled' ? (Array.isArray(submissions.value) ? submissions.value : (submissions.value.submissions || [])) : [];
  ...
  methods: {
    ...
    async handleSubmit() {
      if (!this.submitForm.content.trim()) return;
      ...
      try {
        ...
        if (filePath) submitData.file_path = filePath;
        await api.submitAssignment(submitData);
        store.notify('作业提交成功！', 'success');
    ...
    typeName(t) {
      return { homework: '课后作业', lab_report: '实验报告', comprehensive: '综合案例' }[t] || t;
    },
    typeBadge(t) {
      return { homework: 'bg-blue-50 text-blue-700', lab_report: 'bg-green-50 text-green-700', comprehensive: 'bg-purple-50 text-purple-700' }[t] || 'bg-gray-100 text-gray-600';
    },
    statusName(s) {
